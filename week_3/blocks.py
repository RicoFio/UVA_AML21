...
def nll_forward(target_pred, target_true):
    ...
    # Output
        ...
        scalar
    """
    t1 = target_true*np.log(target_pred)
    t2 = (1-target_true)*np.log(1-target_pred)
    output = (-1/target_pred.shape[0])*(t1+t2).sum()
    return output
...
def nll_grad_input(target_pred, target_true):
    ...
    """
    grad_input = (target_pred - target_true)/(target_pred.shape[0] * target_pred * (1-target_pred))
    return grad_input
